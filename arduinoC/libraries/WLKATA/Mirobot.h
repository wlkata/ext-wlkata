...
class Mirobot_UART{
  public:
		Mirobot_UART(RS485AssicMaster *p = &RS485);
    void init(int addr = -1);//通讯串口初始化 参数：串口指针 地址
		...
		STATUS_MIROBOT getStatus();//读取所有状态
		void waitIdle();//等待空闲状态
		
		void homing(int homingMode = -1);//归航
		void zero();//回零点
		
		void movePose(uint8_t pathMode, bool motionMode, float x, float y, float z, float a, float b, float c);//机械臂位置运动
		void movePoseWithExj(uint8_t pathMode, bool motionMode, float x, float y, float z, float a, float b, float c, float d);//机械臂位置运动，含扩展轴
		void moveArc(bool pathMode, bool motionMode, float x, float y, float z, float r);//机械臂圆弧插补运动
		void moveJoints(bool motionMode, float j1, float j2, float j3, float j4, float j5, float j6);//机械臂关节运动
